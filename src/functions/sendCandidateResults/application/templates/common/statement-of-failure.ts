/* eslint-disable max-len */
export const StatementOfFailureTemplate = `
#Statement of failure to pass practical test
Driving test reference number: {{ applicationReference }}

The candidate with the number shown above has been examined and has FAILED to pass the practical test / test of competence to drive prescribed under the Road Traffic Act (and for the purpose of section 36 of the Road Traffic Offenders Act 1988) in respect of vehicles in the above category / categories.

#How to appeal your driving test
You can appeal if you think your examiner did not follow the regulations when they carried out your test. Find out how to appeal at https://www.gov.uk/driving-test/driving-test-faults-result#appeal-your-driving-test.  Your test result cannot be changed, but you might get a free retest if your appeal is successful. You might need to pay court costs if your appeal does not succeed.

`;

export const StatementOfFailureWelshTemplate = `
#Datganiad o fethiant i basio prawf ymarferol
Cyfeirnod prawf gyrru: {{ applicationReference }}

Arholwyd yr ymgeisydd gyda'r cyfeirnod archebu a ddangosir uchod ac mae wedi METHU i basio'r prawf ymarferol / prawf cymhwysedd i yrru a ragnodir o dan y Ddeddf Traffig Ffyrdd (ac at ddibenion adran 36 Deddf Troseddwyr Traffig Ffyrdd 1988) mewn perthynas â cherbydau yn y categori / categorïau uchod.

#Sut i apelio eich prawf gyrru
Gallwch apelio os credwch na ddilynodd eich arholwr y rheoliadau pan gynhaliodd eich prawf.
Canfyddwch sut i apelio ar https://www.gov.uk/driving-test/driving-test-faults-result#appeal-your-driving-test. Does dim modd newid canlyniad eich prawf, ond efallai y cewch ail brawf am ddim os bydd eich apêl yn llwyddiannus. Efallai y bydd angen i chi dalu costau llys os na fydd eich apêl yn llwyddo.

`;

export const StatementOfFailureVocationalTemplate = `
#Statement of failure to pass practical test
Driving test reference number: {{ applicationReference }}

The candidate with the number shown above has been examined and has FAILED to pass the practical test / test of competence to drive prescribed under the Road Traffic Act (and for the purpose of section 36 of the Road Traffic Offenders Act 1988) in respect of vehicles in the above category / categories.

#How to appeal your driving test
You can appeal if you think your examiner did not follow the regulations as outlined at https://www.gov.uk/guidance/guidance-for-driving-examiners-carrying-out-driving-tests-dt1 when they carried out your test. Contact your local magistrate’s court within 6 months to appeal in England and Wales. If you live in Scotland, contact your local sheriff’s court within 21 days.
Your test result cannot be changed, but you might get a free retest if your appeal is successful. You might need to pay court costs if your appeal does not succeed.

`;

export const StatementOfFailureVocationalWelshTemplate = `
#Datganiad o fethiant i basio prawf ymarferol
Cyfeirnod prawf gyrru: {{ applicationReference }}

Arholwyd yr ymgeisydd gyda'r cyfeirnod archebu a ddangosir uchod ac mae wedi METHU i basio'r prawf ymarferol / prawf cymhwysedd i yrru a ragnodir o dan y Ddeddf Traffig Ffyrdd (ac at ddibenion adran 36 Deddf Troseddwyr Traffig Ffyrdd 1988) mewn perthynas â cherbydau yn y categori / categorïau uchod.

#Sut i apelio eich prawf gyrru
Gallwch apelio os credwch na wnaeth eich arholwr ddilyn y rheoliadau fel yr amlinellwyd yn https://www.gov.uk/guidance/guidance-for-driving-examiners-carrying-out-driving-tests-dt1 pan wnaethant gynnal eich prawf. Cysylltwch â’ch llys ynadon lleol o fewn 6 mis i apelio yng Nghymru a Lloegr. Os ydych yn byw yn yr Alban, cysylltwch â’ch llys siryf lleol o fewn 21 diwrnod.
Does dim modd newid canlyniad eich prawf, ond efallai y cewch ail brawf am ddim os bydd eich apêl yn llwyddiannus. Efallai y bydd angen i chi dalu costau’r llys os na fydd eich apêl yn llwyddo.

`;

export const StatementOfFailureManTemplate = `You can appeal if you think your examiner did not follow the regulations when they carried out your test. Find out how to appeal at https://www.gov.uk/driving-test/driving-test-faults-result#appeal-your-driving-test.  Your test result cannot be changed, but you might get a free retest if your appeal is successful. You might need to pay court costs if your appeal does not succeed.
#Statement of failure to pass practical test
Driving test reference number: {{ applicationReference }}

The candidate with the number shown above has been examined and has FAILED to pass the practical test / test of competence to drive prescribed under the Road Traffic Act 1988 in respect of vehicles in the above category / categories.

#How to appeal your driving test
You can appeal if you think your examiner did not follow the regulations as outlined at https://www.gov.uk/guidance/guidance-for-driving-examiners-carrying-out-driving-tests-dt1 when they carried out your test. Contact your local magistrate’s court within 6 months to appeal in England and Wales. If you live in Scotland, contact your local sheriff’s court within 21 days.
Your test result cannot be changed, but you might get a free retest if your appeal is successful. You might need to pay court costs if your appeal does not succeed.

`;

export const StatementOfFailureManWelshTemplate = `You can appeal if you think your examiner did not follow the regulations when they carried out your test. Find out how to appeal at https://www.gov.uk/driving-test/driving-test-faults-result#appeal-your-driving-test.  Your test result cannot be changed, but you might get a free retest if your appeal is successful. You might need to pay court costs if your appeal does not succeed.
#Datganiad o fethiant i basio prawf ymarferol
Cyfeirnod y prawf gyrru: {{ applicationReference }}
Mae’r ymgeisydd gyda’r rhif a ddangosir uchod wedi’i arholi ac wedi bod yn AFLWYDDIANNUS gyda’r prawf ymarferol / prawf cymhwysedd i yrru a ragnodwyd o dan Ddeddf Traffig Ffyrdd 1988 mewn perthynas â cherbydau yn y categori / categorïau uchod.

#Sut i apelio eich prawf gyrru
Gallwch apelio os credwch na ddilynodd eich archwiliwr y rheoliadau a amlinellwyd ar wefan https://www.gov.uk/guidance/guidance-for-driving-examiners-carrying-out-driving-tests-dt1 pan gwnaethant gynnal eich prawf. Cysylltwch â’ch llys ynadon lleol o fewn 6 mis er mwyn apelio yn Lloegr ac yng Nghymru. Os ydych yn byw yn yr Alban, cysylltwch â’ch llys siryf lleol o fewn 21 diwrnod.
Ni ellir newid canlyniad eich prawf, ond efallai y cewch ail brawf am ddim os bydd eich apêl yn llwyddiannus. Efallai y bydd angen i chi dalu costau llys os na fydd eich apêl yn llwyddiannus.

`;

export const StatementOfFailureMod1Template = `
#Statement of failure to pass practical test
Motorcycle test reference number: {{ applicationReference }}

The candidate with the number shown above has been examined and has failed to pass the manoeuvres test prescribed for the purposes of section 89 of the Road Traffic Act 1988 and for the purposes of section 36 of the Road Traffic Offenders Act 1988.

#How to appeal your motorcycle test
You can appeal if you think your examiner did not follow the regulations as outlined at https://www.gov.uk/guidance/guidance-for-driving-examiners-carrying-out-driving-tests-dt1 when they carried out your test. Contact your local magistrate’s court within 6 months to appeal in England and Wales. If you live in Scotland, contact your local sheriff’s court within 21 days.
Your test result cannot be changed, but you might get a free retest if your appeal is successful. You might need to pay court costs if your appeal does not succeed.

`;

export const StatementOfFailureMod1WelshTemplate = `
#Datganiad o fethiant i basio prawf ymarferol
Cyfeirnod y prawf beiciau modur: {{ applicationReference }}

Mae’r ymgeisydd sydd â’r rhif a ddangosir uchod wedi’i archwilio ac wedi methu â phasio’r prawf symudiadau a ragnodwyd at ddibenion adran 89 o Ddeddf Traffig Ffyrdd 1988 ac at ddibenion adran 36 o Ddeddf Troseddwyr Traffig Ffyrdd 1988.

#Sut i apelio eich prawf beicIau modur
Gallwch apelio os credwch na wnaeth eich arholwr ddilyn y rheoliadau fel yr amlinellwyd yn https://www.gov.uk/guidance/guidance-for-driving-examiners-carrying-out-driving-tests-dt1 pan wnaethant gynnal eich prawf. Cysylltwch â’ch llys ynadon lleol o fewn 6 mis i apelio yng Nghymru a Lloegr. Os ydych yn byw yn yr Alban, cysylltwch â’ch llys siryf lleol o fewn 21 diwrnod.
Does dim modd newid canlyniad eich prawf, ond efallai y cewch ail brawf am ddim os bydd eich apêl yn llwyddiannus. Efallai y bydd angen i chi dalu costau’r llys os na fydd eich apêl yn llwyddo.

`;

export const StatementOfFailureMod2Template = `
#Statement of failure to pass practical test
Motorcycle test reference number: {{ applicationReference }}

The candidate with the number shown above has been examined and has failed to pass the practical test / test of competence to drive prescribed under the Road Traffic Act (and for the purpose of section 36 of the Road Traffic Offenders Act 1988) in respect of vehicles in the above category / categories.

#How to appeal your motorcycle test
You can appeal if you think your examiner did not follow the regulations as outlined at https://www.gov.uk/guidance/guidance-for-driving-examiners-carrying-out-driving-tests-dt1 when they carried out your test. Contact your local magistrate’s court within 6 months to appeal in England and Wales. If you live in Scotland, contact your local sheriff’s court within 21 days.
Your test result cannot be changed, but you might get a free retest if your appeal is successful. You might need to pay court costs if your appeal does not succeed.

`;

export const StatementOfFailureMod2WelshTemplate = `
#Datganiad o fethiant i basio prawf ymarferol
Cyfeirnod y prawf beiciau modur: {{ applicationReference }}

Arholwyd yr ymgeisydd gyda’r cyfeirnod archebu a ddangosir uchod ac mae wedi METHU i basio’r prawf ymarferol / prawf cymhwysedd i yrru a ragnodir o dan y Ddeddf Traffig Ffyrdd (ac at ddibenion adran 36 Deddf Troseddwyr Traffig Ffyrdd 1988) mewn perthynas â cherbydau yn y categori / categorïau uchod.

#Sut i apelio eich prawf beicIau modur
Gallwch apelio os credwch na wnaeth eich arholwr ddilyn y rheoliadau fel yr amlinellwyd yn https://www.gov.uk/guidance/guidance-for-driving-examiners-carrying-out-driving-tests-dt1 pan wnaethant gynnal eich prawf. Cysylltwch â’ch llys ynadon lleol o fewn 6 mis i apelio yng Nghymru a Lloegr. Os ydych yn byw yn yr Alban, cysylltwch â’ch llys siryf lleol o fewn 21 diwrnod.
Does dim modd newid canlyniad eich prawf, ond efallai y cewch ail brawf am ddim os bydd eich apêl yn llwyddiannus. Efallai y bydd angen i chi dalu costau’r llys os na fydd eich apêl yn llwyddo.

`;

export const StatementOfFailureCpcTemplate = `
#Statement of failure to pass CPC Module 4 test
CPC Module 4 reference number: {{ applicationReference }}

The candidate with the number shown above has been examined and FAILED to pass the Module 4 element of the Driver Certificate of Professional Competence (CPC) qualification process. Candidates are assessed against the 5 set topics listed above in deciding whether they are competent to comply with the requirements.

#How to appeal your test
You can appeal if you think your examiner did not follow the regulations as outlined at https://www.gov.uk/guidance/guidance-for-driving-examiners-carrying-out-driving-tests-dt1 when they carried out your test. Contact your local magistrate’s court within 6 months to appeal in England and Wales. If you live in Scotland, contact your local sheriff’s court within 21 days.
Your test result cannot be changed, but you might get a free retest if your appeal is successful. You might need to pay court costs if your appeal does not succeed.

`;

export const StatementOfFailureCpcWelshTemplate = `
#Datganiad o fethiant i basio prawf CPC Modiwl 4
Rhif cyfeirnod CPC Modiwl 4: ((applicationReference))
CPC Module 4 reference number:

#Mae’r ymgeisydd gyda’r nifer a ddangosir uchod wedi cael ei arholi ac wedi METHU pasio elfen Modiwl 4 y broses cymhwyso Tystysgrif Cymhwysedd Proffesiynol y Gyrrwr (CPC). Caiff ymgeiswyr eu hasesu yn ôl y 5 pwnc a restrir uchod wrth benderfynu a ydynt yn gymwys i gydymffurfio â’r gofynion.

Sut i apelio’ch prawf
Gallwch apelio os credwch na wnaeth eich arholwr ddilyn y rheoliadau fel yr amlinellwyd yn https://www.gov.uk/guidance/guidance-for-driving-examiners-carrying-out-driving-tests-dt1 pan wnaethant gynnal eich prawf. Cysylltwch â’ch llys ynadon lleol o fewn 6 mis i apelio yng Nghymru a Lloegr. Os ydych yn byw yn yr Alban, cysylltwch â’ch llys siryf lleol o fewn 21 diwrnod.
Does dim modd newid canlyniad eich prawf, ond efallai y cewch ail brawf am ddim os bydd eich apêl yn llwyddiannus. Efallai y bydd angen i chi dalu costau’r llys os na fydd eich apêl yn llwyddo.
`;
